  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}
      ...
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-sm text-muted-foreground uppercase tracking-wider">Current Plan</h3>
        <Button variant="ghost" size="sm" className="text-xs text-primary hover:text-primary"
          onClick={() => router.push("/dashboard/payment")}>Manage →</Button>
      </div>
      <div className="flex items-center gap-4 mt-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <span className="text-xl font-bold capitalize">{plan}</span>
            {plan !== "starter" && (
          ...
          {usage?.planExpiresAt && (
            <p className="text-xs text-muted-foreground mt-0.5">
              Renews on {format(new Date(usage.planExpiresAt), "MMMM d, yyyy")}
            </p>
          )}
        </div>
      </div>
      {usage && (
        <div className="mt-5 pt-5 border-t border-border">
          <div className="flex justify-between text-xs text-muted-foreground mb-2">
            <span>Message Usage this cycle</span>
