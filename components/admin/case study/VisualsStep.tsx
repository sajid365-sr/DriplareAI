...
export default function VisualsStep<TFormValues extends Record<string, unknown>>({
  ...
  const handleUpload = async (
    ...
    try {
      ...
      const url = await uploadImageToCloudinary(formData, "REMOVED/Website Asset/Case Studies");
    ...
    try {
      const urls = await Promise.all(
        files.map(async (file) => {
          const formData = new FormData();
          formData.append("file", file);
          return uploadImageToCloudinary(formData, "REMOVED/Website Asset/Case Studies/Gallery");
        })
      );
