...
export function ReferralSection({ referralCode }: ReferralSectionProps) {
  ...
  const copyReferral = () => {
    ...
    setCopied(true);
    toast.success(t("profile.referralLinkCopied", "Referral link copied!"));
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14 }}
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        <h3 className="font-semibold">{t("profile.referAndEarn", "Refer & Earn")}</h3>
      </div>
      <p className="text-sm text-muted-foreground mb-4">{t("profile.referDesc", "Share your referral link. Earn rewards when friends sign up.")}</p>
