...
import { ArrowLeft, FileText, Type, Globe, HelpCircle, Upload, Sparkles, Loader2, X } from "lucide-react";
...
export default function CreateChatbot() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const id = data.chatbotId;

      // Upload sources sequentially
      if (text.trim()) {
        await fetch(`/api/chatbots/${id}/sources/text`, {
      ...
      if (url.trim()) {
        try { 
          await fetch(`/api/chatbots/${id}/sources/website`, {
            method: "POST", headers: { "Content-Type": "application/json" },
      ...
      for (const f of files) {
        ...
        try { 
          await fetch(`/api/chatbots/${id}/sources/file`, { method: "POST", body: fd });
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      <Link href="/dashboard/chatbots" className="inline-flex items-center text-sm text-primary hover:underline transition-all" data-testid="back-list">
        <ArrowLeft className="w-4 h-4 mr-1.5" /> Back to Chatbots / Create New Chatbot
      </Link>
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="grid sm:grid-cols-2 gap-6 p-8 bg-card rounded-3xl border border-border/60 shadow-sm">
            <div className="space-y-2">
              ...
              <Input value={name} onChange={(e) => setName(e.target.value)} className="h-11 rounded-xl bg-secondary/30 border-secondary focus:ring-2 focus:ring-primary/20" data-testid="bot-name-input" />
            </div>
            <div className="space-y-2">
                  ...
                  {activeTab === "files" && (
                    <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}>
                      <label className="flex flex-col items-center justify-center border-2 border-dashed border-border/60 rounded-2xl p-16 cursor-pointer hover:border-primary/50 hover:bg-primary/5 transition-all group">
                        ...
                        <div className="text-xs text-muted-foreground mt-2">Supported: .pdf, .docx, .txt — Max 25MB</div>
                  ...
                  {activeTab === "qa" && (
                    <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="flex flex-col items-center justify-center py-12 text-center space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1 max-w-[240px]">You can add specific question and answer pairs after the chatbot is created in the Sources tab.</p>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="p-8 bg-card rounded-3xl border border-border/60 shadow-sm sticky top-24">
            ...
            <div className="space-y-4 mb-8">
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">Manual Text</span>
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className={`font-bold ${url ? "text-primary" : "text-muted-foreground"}`}>{url ? "1 URL" : "None"}</span>
              </div>
            </div>
            ...
            <Button 
              ...
              disabled={creating} 
              onClick={submit} 
              data-testid="create-chatbot-submit"
            >
              {creating ? (
            ...
            <p className="text-[10px] text-center text-muted-foreground mt-4 leading-relaxed">
              By creating a chatbot, you agree to our terms of service. Training may take a few seconds depending on the source size.
            </p>
          </div>
        </div>
      </div>
