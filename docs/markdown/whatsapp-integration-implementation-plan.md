...
এই ডকুমেন্টে REMOVEDAI-এর WhatsApp integration-এর দুইটি flow-এর implementation plan রাখা হলো:
...
REMOVEDAI customer যেন Dashboard থেকে Chatbot create করার পর WhatsApp Business connect করতে পারে, কোনো developer.facebook.com setup না করে।

Customer-facing target flow:

1. Customer logs in to REMOVEDAI.
...
8. REMOVEDAI automatically saves WhatsApp connection metadata.
...
- Simpler, but less control and less observability inside REMOVEDAI.

Recommended for REMOVEDAI production: Option A.
...
n8n WhatsApp Workflow
        |
        v
REMOVEDAI DB + AI + WhatsApp Cloud API
```
...
```text
Reconnect if your WhatsApp number is removed from Meta, permissions are changed, or REMOVEDAI shows a reconnect warning.
```

Bengali:

```text
Meta থেকে WhatsApp number remove হলে, permission পরিবর্তন হলে, অথবা REMOVEDAI reconnect warning দেখালে আবার connect করুন।
```
...
Final REMOVEDAI production flow should be:

1. Customer clicks `Connect WhatsApp`.
2. Meta Embedded Signup opens.
3. Customer connects/verifies number.
4. REMOVEDAI receives phone/WABA metadata.
5. REMOVEDAI saves connection.
6. Meta webhook sends WhatsApp messages to REMOVEDAI.
7. REMOVEDAI/n8n sends AI replies.
